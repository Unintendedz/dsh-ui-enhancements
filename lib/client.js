...
__export(client_exports, {
  apply: () => apply,
  createProjectlessDrafts: () => createProjectlessDrafts,
  ...
  workspaceGroupOrder: () => workspaceGroupOrder,
...
.dsh-workspace-group-row[aria-current=true] { background:color-mix(in srgb,var(--dsw-alias-state-business-primary) 8%,transparent); box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--dsw-alias-state-business-primary) 14%,transparent); border-radius:8px; }
.dsh-workspace-group-row[aria-current=true]:hover { background:color-mix(in srgb,var(--dsw-alias-state-business-primary) 13%,transparent); }
.dsh-workspace-group-row:focus-visible { outline:2px solid var(--dsw-alias-state-business-primary); outline-offset:-2px; }
.dsh-workspace-choice { display:inline-flex; align-items:center; gap:2px; min-width:0; max-width:min(100%,360px); }
.dsh-workspace-choice>button:first-child { min-width:0; max-width:100%; }
.dsh-workspace-choice [data-dsh-projectless-icon] { flex:none; }
.dsh-workspace-clear { display:inline-flex; align-items:center; justify-content:center; flex:none; width:28px; height:28px; padding:0; border:0; border-radius:50%; background:transparent; color:var(--dsw-alias-label-secondary); cursor:pointer; opacity:0; pointer-events:none; }
.dsh-workspace-choice:hover .dsh-workspace-clear,.dsh-workspace-choice:focus-within .dsh-workspace-clear { opacity:1; pointer-events:auto; }
.dsh-workspace-clear:hover { background:var(--dsw-alias-interactive-bg-hover); color:var(--dsw-alias-label-primary); }
.dsh-workspace-choice button:focus-visible { outline:2px solid var(--dsw-alias-state-business-primary); outline-offset:1px; }
@media(hover:none),(pointer:coarse) { .dsh-workspace-clear { opacity:1; pointer-events:auto; width:44px; height:44px; } }
  ...
  const load = () => {
    ...
    request = (async () => {
      try {
        ...
        if (disposed) return;
        for (const [id2, revision] of removed) if (revision <= startedAfterMutation) removed.delete(id2);
...
// src/workspace-sidebar.js
function workspaceGroupOrder(list, workspaces, archivedSessionIds, orderBy = "updated") {
  const archived = new Set(archivedSessionIds);
  const accounted = /* @__PURE__ */ new Set();
  const visible = (session) => session && session.origin !== "subagent" && !archived.has(session.id) && (!session.blank || session.id === list.current);
  const latest = (sessions) => sessions.reduce((time, session) => Number.isFinite(session.updatedAt) ? Math.max(time, session.updatedAt) : time, -Infinity);
  const groups = workspaces.map((workspace) => {
    const members = [];
    for (const id2 of workspace.sessionIds) {
      accounted.add(id2);
      if (visible(list.byId[id2])) members.push(list.byId[id2]);
    }
  ...
  const loose = list.ids.map((id2) => list.byId[id2]).filter((session) => visible(session) && !accounted.has(session.id));
...
function projectlessIcon(React) {
  ...
  return h("svg", {
    ...
    fill: "none",
    "aria-hidden": true,
    "data-dsh-projectless-icon": true
  }, h("path", {
...
function groupHeader(section) {
  ...
  return header?.props?.group && typeof header.props.onToggle === "function" ? header : void 0;
}
function createWorkspaceSidebar(React, NativeBrowser, t) {
  const { createElement: h, cloneElement, useMemo } = React;
  const treeAdapters = /* @__PURE__ */ new WeakMap();
  const rowAdapters = /* @__PURE__ */ new WeakMap();
  function rowAdapter(NativeRow) {
    if (!rowAdapters.has(NativeRow)) rowAdapters.set(NativeRow, function WorkspaceGroupRow(props) {
      ...
      const projectless = props.group.workspaceId === void 0;
      ...
      const decorated = cloneElement(row, {
        ...
        "aria-current": props.group.containsCurrent || void 0,
        ...projectless ? {
        ...
        onKeyDown: (event) => {
          ...
          if (event.key === "Enter" || event.key === " " || event.key === "ArrowRight" && !props.group.expanded || event.key === "ArrowLeft" && props.group.expanded) {
  ...
  function treeAdapter(NativeTree) {
    if (!treeAdapters.has(NativeTree)) treeAdapters.set(NativeTree, function OrderedWorkspaceTree(props) {
      ...
      const ranks = useMemo(() => new Map(workspaceGroupOrder(list, props.workspaces, props.archivedSessionIds, props.orderBy).map((key, index) => [key, index])), [list, props.workspaces, props.archivedSessionIds, props.orderBy]);
      ...
      return mapElements(React, tree, (element) => element, (items) => {
        ...
        const sections = items.map((section) => {
          ...
          children[0] = h(rowAdapter(header.type), {
            ...
            ...props.orderBy === "updated" ? { drag: void 0 } : {}
          });
          return cloneElement(section, { "data-dsh-workspace-group": group.key }, children);
        });
        return sections.sort((a, b) => (ranks.get(groupHeader(a).props.group.key) ?? Infinity) - (ranks.get(groupHeader(b).props.group.key) ?? Infinity));
      });
    });
  ...
  return function WorkspaceSidebar(props) {
    const tree = NativeBrowser({
      ...props,
      t: (key, ...args) => key === "group.ungrouped" ? t("projectless.label") : props.t(key, ...args)
    });
    return mapElements(React, tree, (element) => {
      if (typeof element.type !== "function" || !Array.isArray(element.props.workspaces) || typeof element.props.setGroupExpanded !== "function") return element;
      return h(treeAdapter(element.type), { ...element.props, key: element.key });
    });
  };
}

// src/workspace-choice.js
var VIRTUAL_WORKSPACE = "::dsh-no-workspace";
function createWorkspaceChoice(React, t) {
  const { createElement: h, cloneElement } = React;
  const chips = /* @__PURE__ */ new WeakMap();
  ...
  function chipAdapter(NativeChip) {
    if (!chips.has(NativeChip)) chips.set(NativeChip, function WorkspaceChoiceChip({ projectless, onClear, ...props }) {
      ...
      return h(
        "span",
        { className: "dsh-workspace-choice" },
        cloneElement(button, {}, children),
        onClear && h("button", {
          type: "button",
          className: "dsh-workspace-clear",
          "aria-label": t("projectless.clear"),
          title: t("projectless.clear"),
          onClick: onClear
        }, h(
          "svg",
      ...
      return function WorkspaceChoicePicker(props) {
        ...
        if (!flows.has(NativeFlow)) flows.set(NativeFlow, function WorkspaceChoiceFlow(flowProps) {
          return mapElements(React, NativeFlow(flowProps), (element) => {
            ...
            return cloneElement(element, { items: element.props.items.map((item) => item.id === VIRTUAL_WORKSPACE ? { ...item, icon: projectlessIcon(React) } : item) });
          });
        });
...
// src/projectless.js
var PREFIX = "session-projectless-";
function string(value) {
  if (typeof value !== "string" || !value) throw new TypeError("Expected a non-empty string");
  return value;
}
var codec = (name, parse) => ({ mode: "strict", typeSymbol: `dsh-ui-enhancements#${name}`, schema: { parse } });
var PROJECTLESS_REMOTE = {
  ...
  descriptors: [
    ...
    service: "projectlessConversations",
    namespace: "projectlessConversations",
    method,
...
function isProjectlessDirectory(cwd, root) {
...
function createProjectlessDrafts({ root, sessions, workspaces, prepare, storage, uuid = () => crypto.randomUUID() }) {
  ...
  return {
    connect() {
      ...
      if (summary?.blank && isProjectlessDirectory(summary.cwd, root) && !workspace.archivedSessionIds.includes(remembered) && !workspace.items.some((item) => item.sessionIds.includes(remembered))) return Promise.resolve(remembered);
      retryId ??= uuid();
      inflight = (async () => {
        const location = await prepare(retryId);
        const id2 = await sessions.create(location);
        remembered = id2;
        retryId = void 0;
        try {
          storage?.setItem(key, id2);
        } catch {
        }
        return id2;
      })().finally(() => {
...
function decorateNativeSlot(ctx, key, decorate) {
  return ctx.slots.inject(key, () => {
    const entry = ctx.slots.entries(key).find((item) => item.options.registrant?.startsWith("@deepseek-ai/")) ?? ctx.slots.entries(key)[0];
    ...
    let dispose;
    try {
      dispose = ctx.slots.register({ name: key, priority: Number.MAX_SAFE_INTEGER }, () => null);
    } catch (error) {
...
function registerProjectless(ctx, root, prepare, t) {
  ...
  const connectWrapper = function(id2) {
    return id2 === VIRTUAL_WORKSPACE ? drafts.connect() : connect.call(this, id2);
  };
  const startWrapper = function(id2) {
    if (id2 !== void 0) return start.call(this, id2);
  ...
  try {
    cleanups.push(decorateNativeSlot(ctx, "main.conversation", (Native) => function ProjectlessConversation(props) {
      ...
      const selectWorkspace = async (id2) => {
        setError("");
        try {
          return await props.selectWorkspace(id2);
        } catch (reason) {
      ...
      useEffect(() => {
        if (sessionId !== void 0 || phase !== "ready") return;
        let alive = true;
        setError("");
        navigation.openWorkspace(VIRTUAL_WORKSPACE).catch((reason) => {
      ...
      const renderSlot = (key, owner, options) => {
        ...
        return h(
          ...
          error && h(
            "span",
            { className: "dsh-projectless-error", role: "alert" },
            t("projectless.failed"),
            " ",
            error,
            " ",
            h("button", { type: "button", onClick: () => {
              if (sessionId === void 0) setRetry((n) => n + 1);
      ...
      const renderSlotChain = (key, owner, options) => props.renderSlotChain(
        key,
        owner,
        key === "conversation.composer" && options?.fallback ? { ...options, fallback: choices.composer(options.fallback, choiceOwner) } : options
      );
    ...
    cleanups.push(decorateNativeSlot(ctx, "conversation.session", (Native) => function CarriedDraft(props) {
      ...
      useEffect(() => {
        if (draft && draft !== saved) props.actions.setDraft(draft);
      }, [draft, saved, props.actions]);
...
function codec2(name, parse) {
  return { mode: "strict", typeSymbol: `dsh-ui-enhancements#${name}`, schema: { parse } };
}
var identity = { name: "id", wire: "id", source: "json", codec: codec2("SessionId", id) };
...
var methods = [
  ["listArchived", [], codec2("ArchiveList", parseArchiveList)],
  ["resolveArchivedTitles", [{ name: "ids", wire: "ids", source: "json", codec: codec2("ArchiveIds", (value) => value.map(id)) }], codec2("ArchiveList", parseArchiveList)],
  ["readArchived", [identity], codec2("ArchivePreview", (value) => ({ sessionId: id(value.sessionId), messages: value.messages.map((message) => ({ role: text(message.role), text: text(message.text) })) }))],
  ["restore", [identity], codec2("ArchiveState", archiveState)],
  ["delete", [identity, { name: "confirmed", wire: "confirmed", source: "json", codec: codec2("DeleteConfirmation", (value) => {
    if (value !== true) throw new TypeError("deletion requires confirmation");
    return true;
  }) }], codec2("DeletedSession", (value) => {
...
function registerArchiveEntry(ctx, manager, t, { projectlessRoot } = {}) {
  ...
  function ConversationSettings({ close }) {
    return h(
      ...
      projectlessRoot && h("div", { className: "dsh-archives-settings-row" }, h(
        "div",
        null,
        h("h3", null, t("projectless.label")),
        h("p", null, t("projectless.description")),
...
var inject = ["locale", "remote", "slots", "sessions", "workspaces", "layout", "uiWorkspace"];
...
function apply(ctx) {
  ...
  ctx.effect(async () => {
    const unmount = await ctx.remote.$mount({
      package: NS,
      descriptors: [...PROFILE_PLUGIN_REMOTE.descriptors, ...SESSION_MANAGEMENT_REMOTE.descriptors, ...PROJECTLESS_REMOTE.descriptors]
    });
    const cleanups = [];
    try {
