...
export async function listArchivedSessions(ctx) {
  ...
  const titles = await ctx.sessionQuery.readTitleSnapshots(ids)
  return { items: titles.map(result => {
    if (result.status !== 'fulfilled') return { sessionId: result.sessionId, title: result.sessionId, cwd: '', createdAt: 0, available: false }
    const { session, title } = result.value
    return { sessionId: result.sessionId, title: title?.title ?? result.sessionId, cwd: session.cwd ?? '', createdAt: session.createdAt, available: true }
  }) }
}
...
const initializers = []
for (const method of ['listArchived', 'readArchived', 'restore', 'delete']) Remote(method)(SessionManagementGateway.prototype[method], {
