...
function archivedItem(ctx, id, cache) {
  ...
  const cached = cache.rows.get(id)
  if (cached?.key === headerKey(header)) return Date.now() - cached.at < 60_000 ? cached.item : { ...cached.item, titlePending: true }
  return { ...base, titlePending: true }
}

export async function listArchivedSessions(ctx, cache = titleCache(ctx)) {
  const ids = [...ctx.workspaceRegistry.archivedSessionIds].reverse()
  const present = new Set(ids)
  for (const id of cache.rows.keys()) if (!present.has(id)) cache.rows.delete(id)
  ...
  return { items: ids.map(id => archivedItem(ctx, id, cache)) }
}

export async function resolveArchivedTitles(ctx, ids, cache = titleCache(ctx)) {
  ...
  const items = await Promise.all([...new Set(ids)].map(id => {
    const item = archivedItem(ctx, id, cache)
    ...
    const task = (async () => {
      ...
      if (!before) { cache.rows.delete(id); return { ...item, available: false, titlePending: false } }
      const cached = cache.rows.get(id)
      if (cached?.key === key && cached.revision !== undefined && cached.revision === before.revision) {
        cached.at = Date.now()
        return cached.item
      }
      ...
      // A revision check is a metadata read. An unchanged cold branch keeps its
      // resolved title without replaying the full inherited conversation again.
      if (ctx.workspaceRegistry.archivedSessionIds.includes(id) && before.revision === after?.revision) {
        cache.rows.set(id, {key, item:resolved, revision:before.revision, at:Date.now()})
      }
      return resolved
...
export async function restoreArchivedSession(ctx, id, cache = titleCache(ctx)) {
  sessionId(id)
...
export async function deleteSession(ctx, id, confirmed, handles, cache = titleCache(ctx)) {
  sessionId(id)
  ...
  const result = await restoreArchivedSession(ctx, id, cache)
...
export class SessionManagementGateway extends TypertRemoteService {
  constructor(ctx) {
    ...
    // Cordis supplies a caller-scoped ctx proxy for each invocation. Keep
    // shared title state on the service, not on those transient contexts.
    this.archiveTitles = { rows: new Map(), pending: new Map() }
  ...
  listArchived() { return listArchivedSessions(this.ctx, this.archiveTitles) }
  resolveArchivedTitles(ids) { return resolveArchivedTitles(this.ctx, ids, this.archiveTitles) }
  readArchived(id) { return readArchivedSession(this.ctx, id) }
  restore(id) { return restoreArchivedSession(this.ctx, id, this.archiveTitles) }
  async delete(id, confirmed) {
    sessionId(id)
    if (this.deleting.has(id)) throw new Error('session is already being deleted')
    this.deleting.add(id)
    try { return await deleteSession(this.ctx, id, confirmed, this.handles, this.archiveTitles) }
