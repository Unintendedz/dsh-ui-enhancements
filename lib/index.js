...
import { SessionManagementGateway } from './session-management.js'
import { ProjectlessGateway } from './projectless.js'
export { createProjectlessStorage } from './projectless.js'
export { listArchivedSessions, readArchivedSession, restoreArchivedSession, deleteSession, trackSessionHandles, SessionManagementGateway } from './session-management.js'
...
export function apply(ctx) {
  const profileDir = profileDirFromContext(ctx)
  new PluginToggleGateway(ctx, profileDir, () => ctx.loader.entries())
  if (basename(dirname(profileDir)) !== 'profiles') throw new Error('Cannot resolve the DSH home for managed conversations')
  new ProjectlessGateway(ctx, join(dirname(dirname(profileDir)), 'projectless'))
  ctx.inject(['agents', 'agentLoop', 'sessions', 'workspaceRegistry', 'sessionPersistence', 'sessionQuery', 'sessionProjections', 'sessionProjectionCache'], child => {
    new SessionManagementGateway(child)
  })
}
