...
Version `0.4.0` requires DSH `0.1.5-rc.1` and its JSONL session backend. Restart DSH after installation so the plugin can track each conversation's native lifecycle.
...
```sh
dsh plugin --profile web add github:Unintendedz/dsh-ui-enhancements#v0.4.0
```
